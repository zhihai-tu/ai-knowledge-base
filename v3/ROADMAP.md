...
- `workflows/graph.py` 改为 review 后三路条件路由 `route_after_review`：通过 → save；未通过且 `iteration < MAX_ITERATIONS(3)` → revise（按反馈改写 analyses 后 `add_edge("revise","review")` 形成重做循环）；未通过且已达上限 → human_flag（`add_edge("human_flag", END)` 异常终点，写入 pending_review/）。node/edge 引用全部切换为 reviewer.py / reviser.py / human_flag.py。已验证：`py_compile` 通过、`build_graph()` 编译、Router 六分支断言、stub 节点动态跑通 FAIL 路径（review×3 → revise×2 → human_flag → END）与 PASS 路径（→ save）。待办：真实 LLM 端到端验证。
- `workflows/reviewer.py` 移除 `iteration >= MAX_ITERATIONS` 强制通过逻辑（此前 force-pass 使「未通过且 iter≥3 → human_flag」分支不可达）：现在未通过一律如实递增 iteration，循环出口完全交 graph 路由（<3 → revise 重做；≥3 → human_flag 人工介入）；`MAX_ITERATIONS` 导入随之移除（仅 state.py 保留定义供路由使用）；`iteration` 字段与模块 docstring 同步更新。已验证：`py_compile`、iteration=3 时不再 force-pass 且 iteration 正常递增为 4、真实 review/revise/human_flag 全链路跑通（review×3→revise×2→human_flag，pending_review 落盘 iterations_used=3）。LLM 调用失败/缺 scores 时自动通过行为保持不变。
- 修订节点独立模块 `workflows/reviser.py`：新增 `revise_node`（读取 `state["analyses"]` 与 `state["review_feedback"]`，反馈注入修订 prompt，LLM 返回改写后的 analyses，temperature=0.4 允许创造性改写；analyses 或 feedback 为空时返回 `{}` 跳过；按 source_id 合并改写结果、保持原顺序，调用失败/输出非法时保留原 analyses 不阻塞流程；成本累计进 `state.cost_tracker`）。已验证：`py_compile` 通过，monkeypatch 覆盖 6 分支（空 analyses/空 feedback/正常改写+temperature 与成本断言/调用失败/输出非数组/缺 id 条目沿用）。待办：接入 `graph.py` 编排。
